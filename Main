...
CHUNK_SIZE = 1 * 1024 * 1024  # 1MB chunks for better compatibility
...
# Fixed high-performance download function
async def turbo_download(client, message, file_path):
    ...
    # Use the correct download method without chunk_size parameter
    download_path = await client.download_media(
        message,
        file_name=file_path,
        progress=download_progress_callback,
        progress_args=(start_time, downloaded_size, "download")
    )
...
async def download_progress_callback(current, total, start_time, downloaded_size, operation):
    """Download progress callback for speed calculation"""
...
# Fixed high-performance upload function
async def turbo_upload(client, chat_id, file_path, file_name, caption, thumb):
    ...
    elif file_path.endswith(('.jpg', '.jpeg', '.png', '.bmp', '.webp')):
    ...
    elif file_path.endswith(('.mp3', '.m4a', '.flac', '.wav', '.ogg')):
...
async def start_command(client, message: Message):
    ...
    await message.reply_text(
        ...
        "• 🔥 **Smart chunking** for extreme throughput\n"
