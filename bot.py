...
async def ultra_fast_process_file(client, message: Message, target_message: Message):
    ...
    try:
        ...
        if existing_message_id:
            ...
            download_path = await ultra_fast_download(client, target_message, temp_path, None)
...
# CHANNEL SETUP COMMAND - FIXED STRING
...
async def setup_channel_command(client, message: Message):
    ...
    help_text = (
        "🔧 **CHANNEL SETUP GUIDE**\n\n"
        f"**Current Channel:** `{STORAGE_CHANNEL}`\n\n"
        "**Step-by-Step Setup:**\n\n"
        "1. **Create a Telegram Channel**\n"
        "   • Go to Telegram → New Channel\n"
        "   • Give it a name (e.g., 'My File Storage')\n"
        "   • Make it **Public** (recommended)\n\n"
        f"2. **Add Bot as Admin**\n"
        f"   • Go to Channel → Edit → Administrators\n"
        f"   • Add @{bot_username} as admin\n"
        ...
        "   • Copy the exact username including @\n\n"
        "4. **Update .env File**\n"
        "   ```\n"
        f"   STORAGE_CHANNEL=@your_channel_username\n"
        "   ```\n\n"
    ...
    try:
        ...
        if not channel_access:
            await status_msg.edit_text(
                ...
                "3. ✅ Bot has post permissions\n"
                "4. ✅ Channel is accessible\n\n"
                "Use `/setup_channel` for setup guide"
            )
            return
        
        await status_msg.edit_text(
            f"✅ **Channel Access Verified!**\n\n"
            f"**Channel:** `{STORAGE_CHANNEL}`\n"
            "**Status:** Ready to store files! 🎯\n\n"
            "You can now use `/rename` command!"
        )
        ...
        await status_msg.edit_text(
            ...
            f"**Error:** {str(e)}\n\n"
            f"**Current Channel:** `{STORAGE_CHANNEL}`\n\n"
            "Use `/setup_channel` for help"
        )

# SIMPLE THUMBNAIL COMMANDS
...
async def view_thumbnail_command(client, message: Message):
    ...
    if thumbnail_path and os.path.exists(thumbnail_path):
        await message.reply_photo(thumbnail_path, caption="🖼️ Your thumbnail")
    else:
...
async def storage_status_command(client, message: Message):
    ...
    await message.reply_text(
        ...
        f"**Max Size:** {format_size(MAX_FILE_SIZE)}\n\n"
        f"**Commands:**\n"
        f"• `/test_channel` - Test access\n"
        f"• `/setup_channel` - Setup guide\n"
        f"• `/rename` - Rename files"
    )
...
async def start_command(client, message: Message):
    bot_username = (await client.get_me()).username
    channel_access = await verify_channel_access(client)
    channel_status = "✅ Connected" if channel_access else "❌ Setup Required"
    
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔧 Setup Channel", callback_data="setup_channel")],
        [InlineKeyboardButton("💾 Storage Status", callback_data="storage_status")],
    ...
    await message.reply_text(
        ...
        f"**Channel:** `{STORAGE_CHANNEL}`\n\n"
        f"**How to use:**\n"
        f"1. Reply to any file\n"
        f"2. Send `/rename new_name.ext`\n\n"
        f"**Need setup?** Use /setup_channel",
        reply_markup=keyboard,
        parse_mode=ParseMode.MARKDOWN
    )

# SIMPLE CALLBACK HANDLERS
@app.on_callback_query(filters.regex("setup_channel"))
async def setup_channel_callback(client, callback_query):
    await callback_query.answer()
    await setup_channel_command(client, callback_query.message)
...
async def thumbnail_settings_callback(client, callback_query):
    ...
    await callback_query.message.edit_text(
        ...
        "Thumbnails are used for video/audio/document files."
    )
...
# FLOOD WAIT FIXED CLIENT INITIALIZATION
try:
    app = Client(
        "ultra_fast_bot",
        api_id=API_ID,
    ...
    print("✅ Pyrogram client configured")
except Exception as e:
    print(f"❌ Client configuration error: {e}")
    sys.exit(1)

# START BOT WITH FLOOD WAIT PROTECTION
if __name__ == "__main__":
    ...
    print("🛡️  Flood wait protection: ACTIVE")
    print("⏰ If flood wait occurs, bot will wait automatically")
    print("🎯 Bot starting...")
    
    try:
        # Start with flood wait handling
        app.run()
    except FloodWait as e:
        wait_time = e.value
        print(f"⏳ Flood wait detected on startup: {wait_time} seconds")
        print(f"🕒 Please wait {wait_time} seconds and try again")
        print("💡 This is normal if you restarted the bot too quickly")
    except Exception as e:
