...
async def turbo_upload(client, chat_id, file_path, file_name, caption, thumb):
    ...
    if file_path.endswith(('.mp4', '.mkv', '.avi', '.mov')):
        message = await client.send_video(
            ...
            progress_args=(start_time, uploaded_size, file_size, "upload")
        )
    else:
...
async def progress_callback(current, total, start_time, uploaded_size, file_size, operation):
    """Progress callback for speed calculation"""
    elapsed_time = time.time() - start_time
...
async def start_command(client, message: Message):
    ...
    await message.reply_text(
        ...
        "• `/speed_test` - Test connection speed\n"
...
# Extreme speed test command
@app.on_message(filters.command("speed_test"))
async def speed_test_command(client, message: Message):
    if is_message_processed(message.id):
        return
    mark_message_processed(message.id)
    
    status_msg = await message.reply_text("🚀 **Testing connection speed...**")
    ...
    try:
        ...
        await status_msg.edit_text("📥 **Testing download speed...**")
        download_speed = st.download() / 1_000_000  # Mbps
        
        await status_msg.edit_text("📤 **Testing upload speed...**")
        upload_speed = st.upload() / 1_000_000  # Mbps
        
        # Update speed stats
        download_speed_mb = download_speed / 8  # Convert to MB/s
        upload_speed_mb = upload_speed / 8  # Convert to MB/s
        update_speed_stats(download_speed_mb, upload_speed_mb)
        
        await status_msg.edit_text(
            f"📊 **SPEED TEST RESULTS**\n\n"
            f"📥 **Download:** {download_speed:.2f} Mbps\n"
            f"📤 **Upload:** {upload_speed:.2f} Mbps\n"
            f"💾 **Download:** {download_speed_mb:.2f} MB/s\n"
            f"💽 **Upload:** {upload_speed_mb:.2f} MB/s\n\n"
            f"**Status:** ✅ **EXTREME SPEED READY**"
        )
...
async def extreme_process_file_rename(client, message: Message, target_message: Message):
    ...
    try:
        ...
        await turbo_upload(client, target_message.chat.id, file_path, new_name, user_caption, thumbnail_path)
