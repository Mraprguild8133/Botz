...
# FLOOD WAIT HANDLER FOR CLIENT INITIALIZATION
async def initialize_client_with_retry():
    """Initialize client with flood wait handling"""
    max_retries = 3
    retry_count = 0
    ...
    while retry_count < max_retries:
        try:
            ...
            retry_count += 1
            wait_time = e.value
            print(f"⏳ Flood wait detected: {wait_time} seconds (Attempt {retry_count}/{max_retries})")
            
            if wait_time > 300:  # If wait time is more than 5 minutes
                print(f"❌ Flood wait too long ({wait_time}s). Please wait and try again later.")
                return None
